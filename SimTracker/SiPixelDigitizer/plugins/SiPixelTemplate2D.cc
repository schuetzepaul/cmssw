//
//  SiPixelTemplate2D.cc  Version 1.04 
//
...
// V1.04 - Set pixel size during initialization
...
#else
//#include "SiPixelTemplate2D.h"
#include "SimTracker/SiPixelDigitizer/interface/SiPixelTemplate2D.h"
...
bool SiPixelTemplate2D::pushfile(int filenum)
{
	...
	if(in_file.is_open()) {
        ...
        // Set the pixel size now to make it available to the re-weighting code before a 2D template is requested
        
        xsize_ = theCurrentTemp.head.xsize;
        ysize_ = theCurrentTemp.head.ysize;
        zsize_ = theCurrentTemp.head.zsize;
...
//	std::vector <float> xrms(4), xgsig(4), xrmsc2m(4), xgsigc2m(4);
	...
	//std::cout << "X Hit: " << xhit << "  Y Hit: " << yhit << std::endl; 
...
// Determine the size of this template    
	...
	if(index_id_ < 0 || index_id_ >= (int)thePixelTemp_.size()) {
		throw cms::Exception("DataCorrupt") << "SiPixelTemplate2D::interpolate can't find the fucking needed template ID = " << id 
...
// Check angle limits and et up interpolation parameters  
	float usedCotalpha = 0;

	if(cotalpha < cotalpha0_) {
	  usedCotalpha = cotalpha0_;
		success_ = false;
		jx0_ = 0;
		jx1_ = 1;
		adcota_ = 0.f;
	} else if(cotalpha > cotalpha1_) {
	  usedCotalpha = cotalpha1_;
	        success_ = false;
		jx0_ = Nxx_ - 1;
		jx1_ = jx0_ - 1;
		adcota_ = 0.f;
	   ...
	   usedCotalpha = cotalpha0_ + jx0_*deltacota_;
	...
	std::cout << "cotalpha entered was: " << cotalpha << std::endl;
	std::cout << "cotalpha used is: " << usedCotalpha << std::endl;
...
// Interpolate things in cot(alpha)-cot(beta)	
	//std::cout << "Index ID: " << index_id_ << "Entry: " << iy0_ << "   " << jx0_ << std::endl;
...
// First find the limits of the indices for non-zero pixels
	...
	std::cout << "Min pixel in templates y:" << imin << std::endl;
	std::cout << "Max pixel in templates y:" << imax << std::endl;
	std::cout << std::endl;
	std::cout << "Min pixel in templates x:" << jmin << std::endl;
	std::cout << "Max pixel in templates x:" << jmax << std::endl;

	std::cout << "Pix x:  " << pixx << "  Pix Y:  " << pixy << std::endl;
	std::cout << "dy is: " << dy << std::endl;
	std::cout << "dx is: " << dx << std::endl;
	//std::cout << "k0 is: " << k0 << std::endl;
	//std::cout << "ddy is: " << ddy << std::endl;
	//std::cout << "ady is: " << ady << std::endl;
...
// Loop over the non-zero part of the template index space and interpolate
	...
	//if(flip_y)
	  //std::cout << "flip_y is true!" << std::endl;
